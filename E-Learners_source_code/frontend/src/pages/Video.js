...
export default function Video() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8000/get_video/?chapterid=${chapterid}&videonum=${videoNum}`
      )
      .then((res) => {
        data = res.data;
        // this.setState({
  ...
  function onProgress(state) {
    ...
    let data;
    axios
      .post("http://localhost:8000/save_video_progress/", {
        user: userid,
        progress: x,
        tutorial: tutorial,
      })
  ...
  function closeVideo() {
    navigate(
      `/CareerTracks/${localStorage.getItem(
        "trackid"
  ...
  return (
    ...
    <div>
      <div className="videoHeader">
        <div className="video-chapter">{chapter_title}</div>
        <button className="closeVideobtn" onClick={closeVideo}>
          {" "}
          Exit{" "}
        </button>
      </div>
          ...
          <div className="videoPlayer">
            {video.endsWith("pdf") === true ? (
              <div className="pdf-reader">
                <div className="videobtn2">
                  <button className="prevVideobtn" onClick={goToPrevPage}>
                    Prev
                  </button>
                  <p>
                    Page {pageNumber} of {numPages}
                  </p>
                  <button className="nextVideobtn" onClick={goToNextPage}>
                    Next
                  </button>
                </div>
            ...
            <div className="videobtn">
              <button className="prevVideobtn" onClick={prevVideo}>
                {" "}
                Previous{" "}
              </button>
              <p>
  ...
  function TutorialsListEnrolled() {
    ...
    return (
      <div className="tutorials-container">
        <div className="courseRecom-header">Tutorials ({tutorials.length})</div>
